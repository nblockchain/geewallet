...
type InvoiceManagement (account: NormalUtxoAccount, password: string) =
    ...
    member self.CreateInvoice (amountInSatoshis: uint64) (description: string) =
        ...
        let invoiceFields =
            {
                TaggedFields.Fields =
                    [
                        ...
                        TaggedField.ExpiryTaggedField (DateTimeOffset.UtcNow.AddHours (1.))
        ...
        let paymentRequest = 
            UnwrapResult (PaymentRequest.TryCreate (network, LNMoney.Satoshis amountInSatoshis |> Some, DateTimeOffset.UtcNow, invoiceFields, nodeMasterPrivKey.RawExtKey().PrivateKey)) "failed to create the invooice"
