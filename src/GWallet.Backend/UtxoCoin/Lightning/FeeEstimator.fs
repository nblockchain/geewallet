...
type FeeEstimator = {
    ...
    static member Create() = async {
        let averageFee (feesFromDifferentServers: List<decimal>): decimal =
            ...
            avg
        let estimateFeeJob =
            ElectrumClient.EstimateFee 2 // same confirmation target as in UtxoCoinAccount
        let! btcPerKb =
            Server.Query Currency.BTC (QuerySettings.FeeEstimation averageFee) estimateFeeJob None
