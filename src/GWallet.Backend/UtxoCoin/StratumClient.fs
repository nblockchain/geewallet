...
type public ElectrumServerReturningImproperJsonResponseException
    (
        message: string,
        innerEx: Exception
    ) =
...
type public ElectrumServerReturningInternalErrorException
    ...
    inherit ElectrumServerReturningErrorException
        (
            message,
            code,
            originalRequest,
    ...
    static member private DeserializeInternal<'T>(result: string) : 'T =
        ...
        let deserializedValue =
            ...
            | :? Newtonsoft.Json.JsonSerializationException as serEx ->
                let newEx =
                    ElectrumServerReturningImproperJsonResponseException(
                        failedDeserMsg,
                        serEx
                    )
#if !DEBUG
                Infrastructure.ReportWarning newEx |> ignore<bool>
#endif
                raise newEx
    ...
    static member public Deserialize<'T>(result: string) : 'T =
        ...
        | false ->
            ...
            <| ServerMisconfiguredException(
                SPrintF1 "Server's reply was not valid json: %s" result
            )
        | true -> StratumClient.DeserializeInternal result
