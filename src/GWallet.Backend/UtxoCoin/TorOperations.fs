...
module internal TorOperations =
    let GetRandomTorDirectory() =
        match Caching.Instance.GetServers Currency.Tor |> Shuffler.Unsort |> Seq.tryHead with
        | Some server ->
    ...
    let internal GetTorDirectory(): Async<TorDirectory> =
        async {
            return! FSharpUtil.Retry<TorDirectory, NOnionException>
                (fun _ -> TorDirectory.Bootstrap (GetRandomTorDirectory()))
