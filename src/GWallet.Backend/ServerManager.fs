...
module ServerManager =
    ...
    let UpdateServersFile () =
        ...
        let fromElectrumServerToGenericServerDetails
            (es: UtxoCoin.ElectrumServer)
            =
            match es.UnencryptedPort with
            | None ->
                failwith "filtering for non-ssl electrum servers didn't work?"
        ...
        let electrumBtcServers =
            UtxoCoin.ElectrumServerSeedList.ExtractServerListFromElectrumRepository
                btc

        let eyeBtcServers =
            UtxoCoin.ElectrumServerSeedList.ExtractServerListFromWebPage btc

        let baseLineBtcServers =
            match baseLineServers.TryGetValue btc with
            | true, baseLineBtcServers -> baseLineBtcServers
            | false, _ ->
                failwith
                <| SPrintF1 "There should be some %A servers as baseline" btc

        let allBtcServers =
            Seq.append electrumBtcServers eyeBtcServers
            |> Seq.map fromElectrumServerToGenericServerDetails
            |> Seq.append baseLineBtcServers

        let ltc = Currency.LTC

        let electrumLtcServers =
            UtxoCoin.ElectrumServerSeedList.ExtractServerListFromElectrumRepository
                ltc

        let eyeLtcServers =
            UtxoCoin.ElectrumServerSeedList.ExtractServerListFromWebPage ltc

        let baseLineLtcServers =
            match baseLineServers.TryGetValue ltc with
            | true, baseLineLtcServers -> baseLineLtcServers
            | false, _ ->
                failwith
                <| SPrintF1 "There should be some %A servers as baseline" ltc

        let allLtcServers =
            Seq.append electrumLtcServers eyeLtcServers
            |> Seq.map fromElectrumServerToGenericServerDetails
        ...
        for KeyValue (currency, servers) in baseLineServers do
            Infrastructure.LogInfo (
                SPrintF2
                    "%i %A servers from baseline JSON file"
                    (servers.Count ())
                    currency
            )
            ...
            | Currency.BTC ->
                Infrastructure.LogInfo (
                    SPrintF1
                        "%i BTC servers from electrum repository"
                        (electrumBtcServers.Count ())
                )

                Infrastructure.LogInfo (
                    SPrintF1
                        "%i BTC servers from bitcoin-eye"
                        (eyeBtcServers.Count ())
                )
            | Currency.LTC ->
                Infrastructure.LogInfo (
                    SPrintF1
                        "%i LTC servers from electrum repository"
                        (electrumLtcServers.Count ())
                )

                Infrastructure.LogInfo (
                    SPrintF1
                        "%i LTC servers from bitcoin-eye"
                        (eyeLtcServers.Count ())
                )
            | _ -> ()
        ...
        for KeyValue (currency, servers) in filteredOutServers do
            Infrastructure.LogInfo (
                SPrintF2 "%i %A servers total" (servers.Count ()) currency
            )
    ...
    let private GetDummyBalanceAction (currency: Currency) servers =
            ...
            if currency.IsUtxo () then
                let scriptHash =
                    ...
                    | Currency.BTC ->
                        ...
                        UtxoCoin.Account.GetElectrumScriptHashFromPublicAddress
                            currency
                            SATOSHI_ADDRESS
                    | Currency.LTC ->
                        ...
                        UtxoCoin.Account.GetElectrumScriptHashFromPublicAddress
                            currency
                            LTC_GENESIS_BLOCK_ADDRESS
                    | _ -> failwith <| SPrintF1 "Currency %A not UTXO?" currency

                let utxoFunc electrumServer =
                    async {
                        let! bal =
                            UtxoCoin.ElectrumClient.GetBalance
                                scriptHash
                                electrumServer
                        ...
                        let! balance =
                            Async.AwaitTask (
                                web3.Eth.GetBalance.SendRequestAsync
                                    ETH_GENESISBLOCK_ADDRESS
                            )

                        return balance.Value |> decimal
                    }

                Ether.Server.GetServerFuncs web3Func servers currency |> Some
        ...
        | Some queryFuncs ->
            async {
                try
                    let! _ =
                        tester.Query testingSettings (queryFuncs |> List.ofSeq)

                    return ()
                with
                | :? NoneAvailableException -> return ()
            }
            |> Some
        | _ -> None
            ...
            seq {
                for currency in Currency.GetAll () do
                    ...
                    if not (currency.IsEthToken ()) then
                        let serversForSpecificCurrency =
                            Caching.Instance.GetServers currency

                        match GetDummyBalanceAction
                                  currency
                                  serversForSpecificCurrency with
                        | None -> ()
                        | Some job -> yield job
            }

        Async.Parallel jobs |> Async.RunSynchronously |> ignore<array<unit>>
