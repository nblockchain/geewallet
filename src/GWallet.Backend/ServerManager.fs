...
module ServerManager =
    ...
    let UpdateServersFile () =
        ...
        let baseLineServers =
            Config.ExtractEmbeddedResourceFileContents ServerRegistry.ServersEmbeddedResourceFileName
        ...
        let baseLineBtcServers =
            ...
            | false, _ -> failwith <| SPrintF1 "There should be some %A servers as baseline" btc
        ...
        let baseLineLtcServers =
            ...
            | false, _ -> failwith <| SPrintF1 "There should be some %A servers as baseline" ltc
    ...
    let private GetDummyBalanceAction (currency: Currency) servers =
        ...
        | Some queryFuncs ->
            async {
                try
                    let! _ = tester.Query testingSettings (queryFuncs |> List.ofSeq)
                    return ()
    ...
    let private UpdateBaseline () =
        ...
        | Some serversInJson -> File.WriteAllText (ServerRegistry.ServersEmbeddedResourceFileName, serversInJson)
