...
type internal UnhandledSocketException =
    ...
    new (socketErrorCode: int, innerException: Exception) =
        { inherit Exception (SPrintF1
                                 "Backend not prepared for this SocketException with ErrorCode[%i]"
                                 socketErrorCode,
                             innerException) }
    ...
    let FindExceptionToRethrow
        ...
        | Some socketException ->
            if socketException.ErrorCode = int SocketError.ConnectionRefused then
                ServerRefusedException (newExceptionMsg, ex) :> Exception
                |> Some
                ...
                ServerTimedOutException (newExceptionMsg, ex) :> Exception
                |> Some
            ...
            elif socketException.ErrorCode = int
                                                 SocketError.AddressFamilyNotSupported then
                ServerUnreachableException (newExceptionMsg, ex) :> Exception
                |> Some
            ...
            // -1!?! WTF, mono bug in v6.4.0? see https://sentry.io/organizations/nblockchain/issues/1261821968/
                ...
                ServerClosedConnectionEarlyException (newExceptionMsg, ex)
                :> Exception
                |> Some
            ...
            // (the version when Xamarin will work the .NETCore BCL)
                ...
                BuggyExceptionFromOldMonoVersion (newExceptionMsg, ex)
                :> Exception
                |> Some

            else
                UnhandledSocketException (socketException.ErrorCode, ex)
                :> Exception
                |> Some
