...
type JsonRpcTcpClient(host: string, port: uint32) =
    ...
    let ResolveAsync(hostName: string) : Async<Option<IPAddress>> =
        async {
    ...
    let exceptionMsg =
        "JsonRpcSharp faced some problem when trying communication"

    let ResolveHost() : Async<IPAddress> =
        async {
            try
                let! maybeTimedOutipAddress =
                    ResolveAsync host
                    |> FSharpUtil.WithTimeout Config.DEFAULT_NETWORK_TIMEOUT
                ...
                | Some ipAddressOption ->
                    ...
                    | Some ipAddress ->
                        if ipAddress.ToString().StartsWith("127.0.0.") then
                            let msg =
                                SPrintF2
                                    "Server '%s' resolved to localhost IP '%s'"
                                    host
                                    (ipAddress.ToString())

                            return
                                raise
                                <| ServerNameResolvedToInvalidAddressException(
                                    msg
                                )
                        else
                            return ipAddress
                    | None ->
                        return
                            ...
                            <| ServerCannotBeResolvedException(
                                SPrintF1
                                    "DNS host entry lookup resulted in no records for %s"
                                    host
                            )
                | None ->
                    return
                        raise
                        <| TimeoutException(
                            SPrintF2 "Timed out connecting to %s:%i" host port
                        )
            with
            | :? TimeoutException ->
                ...
                | Some socketException ->
                    if socketException.ErrorCode = int SocketError.HostNotFound
                       ...
                       || socketException.ErrorCode = int SocketError.TryAgain then
                        return
                            raise
                            <| ServerCannotBeResolvedException(exceptionMsg, ex)

                    return
                        raise
                        <| UnhandledSocketException(
    ...
    let rpcTcpClientInnerRequest =
        let tcpClient =
            JsonRpcSharp.TcpClient.JsonRpcClient(
                ResolveHost,
                int port,
    ...
    member __.Request(request: string) : Async<string> =
        async {
            try
                let! stringOption =
                    rpcTcpClientInnerRequest request
                    |> FSharpUtil.WithTimeout Config.DEFAULT_NETWORK_TIMEOUT
                ...
                let str =
                    ...
                    | None ->
                        ...
                        <| ServerTimedOutException
                            "Timeout when trying to communicate with UtxoCoin server"
            ...
            | :? JsonRpcSharp.TcpClient.CommunicationUnsuccessfulException as ex ->
                return
                    raise <| CommunicationUnsuccessfulException(ex.Message, ex)
