...
type ResourceUnavailabilityException
    (
        message: string,
        innerOrLastException: Exception
    ) =
    inherit Exception(message, innerOrLastException)

type private TaskUnavailabilityException
    (
        message: string,
...
type private ServerUnavailabilityException
    (
        message: string,
...
type private NotEnoughAvailableException
    (
        message: string,
        lastException: Exception
    ) =
...
type ResultInconsistencyException
    (
        totalNumberOfSuccesfulResultsObtained: int,
            ...
            | ex ->
                ...
                let report =
                    ...
                    && cancelState.SafeDo(fun state ->
                        isLateEnoughToReportProblem state.Value
                    )
...
type FaultTolerantParallelClient<'K, 'E when 'K: equality and 'K :> ICommunicationHistory and 'E :> Exception>
    ...
    do
        if typeof<'E> = typeof<Exception> then
            raise
            <| ArgumentException(
                "'E cannot be System.Exception, use a derived one",
                "'E"
            )
    ...
    let rec WhenSomeInternal
        ...
        async {
            if startedTasks = List.Empty then
                ...
                let jobToWaitForFirstFinishedTask =
                    ServerTask.WhenAny startedTasks

                let! fastestTask = jobToWaitForFirstFinishedTask

                let restOfTasks =
                    startedTasks.Where(fun task -> not(task = fastestTask))
                    |> List.ofSeq
                            ...
                            cancelState.SafeDo(fun state ->
                                let resultingTask =
                                    ...
                                    | Alive cancelSources ->
                                        ...
                                        state.Value <-
                                            Alive(
                                                newTask.CancellationTokenSource
                                                :: cancelSources
                                            )
                ...
                let returnWithConsistencyOf
                    (minNumberOfConsistentResultsRequired: Option<uint32>)
                    cacheMatchFunc
                    =
                    async {
                        ...
                        |
                            ...
                            match
                                minNumberOfConsistentResultsRequired,
                                cacheMatchFunc
                                with
                            | None, None ->
                            ...
                            | _ ->
                                return
                                    failwith
                                        "should be either both None or both Some!"
                    }
                ...
                | Some(SpecificNumberOfConsistentResponsesRequired number) ->
                    return!
                        returnWithConsistencyOf
                            (Some number)
                            ((fun _ -> false) |> Some)
                | Some
                    (
                        OneServerConsistentWithCertainValueOrTwoServers cacheMatchFunc
                    ) ->
                    return!
                        returnWithConsistencyOf (Some 2u) (Some cacheMatchFunc)
                | None ->
                    if newRestOfTasks.Length = 0 then
                        ...
                        Infrastructure.LogDebug(
                            SPrintF1
                                ...
                                (100.
                                 * (float(
                                     newFailedFuncs.Length + newResults.Length
                                 ))
        ...
        let startedTasks, jobsToLaunchLater =
            ...
            | None ->
                ...
                <| TaskCanceledException
                    "Found canceled when about to launch more jobs"
            | Some(firstJobsToLaunch, jobsToLaunchLater) ->
                ...
                cancelState.SafeDo(fun state ->
                    ...
                    | Canceled _ ->
                        raise
                        <| TaskCanceledException
                            "Found canceled when about to launch more tasks"
                    | Alive currentList ->
                        let startedTasks =
                            firstJobsToLaunch
                            |> List.map(fun job -> LaunchAsyncJob job)

                        let newCancelSources =
                            startedTasks
                            |> List.map(fun task -> task.CancellationTokenSource
                            )

                        state.Value <-
                            Alive(List.append currentList newCancelSources)
        ...
        async {
            if not(funcs.Any()) then
                return
                    raise
                    <| ArgumentException(
                        "number of funcs must be higher than zero",
                        "funcs"
                    )

            let howManyFuncs = uint32 funcs.Length

            let numberOfParallelJobsAllowed =
                int settings.NumberOfParallelJobsAllowed
            ...
            | InconsistentOrNotEnoughResults executedServers ->
                let failedFuncs =
                    ...
                    |> List.map(fun unsuccessfulServer ->
                        unsuccessfulServer.Server
                    )
                ...
                if executedServers.SuccessfulResults.Length = 0 then
                    if retries = settings.NumberOfRetries then
                        ...
                        return
                            raise
                            <| NoneAvailableException("Not available", firstEx)
                    else
                        return!
                    ...
                    let totalNumberOfSuccesfulResultsObtained =
                        executedServers.SuccessfulResults.Length
                    ...
                    | Some
                        ...
                        let resultsOrderedByCount =
                            MeasureConsistency executedServers.SuccessfulResults
        ...
        let sortedWorkingServers =
            List.sortBy
                (fun server ->
                    ...
                    | None ->
                        failwith
                            "previous filter didn't work? should get working servers only, not lacking history"
                    | Some historyInfo ->
                        match historyInfo.Status with
                        | Fault _ ->
                            failwith
                                "previous filter didn't work? should get working servers only, not faulty"
                        | _ -> historyInfo.TimeSpan
                )
                workingServers

        let serversWithNoHistoryServers =
            List.filter
                (fun server -> server.Details.CommunicationHistory.IsNone)
                servers
            ...
            List.sortBy
                (fun server ->
                    ...
                    | Some historyInfo ->
                        ...
                        | _ ->
                            failwith
                                "previous filter didn't work? should get faulty servers only, not working ones"
                )
                faultyServers

        if mode = ServerSelectionMode.Fast then
            List.append
                sortedWorkingServers
                (List.append serversWithNoHistoryServers sortedFaultyServers)
        else
    ...
    member self.Query<'R when 'R: equality>
        (settings: FaultTolerantParallelClientSettings<'R>)
        (servers: List<Server<'K, 'R>>)
        : Async<'R> =
