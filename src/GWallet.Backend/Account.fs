...
module Account =
    ...
    let private GetShowableBalanceInternal
        ...
        | :? UtxoCoin.IUtxoAccount as utxoAccount ->
            if not(account.Currency.IsUtxo()) then
                failwith
                <| SPrintF1
                    "Currency %A not Utxo-type but account is? report this bug (balance)"
                    account.Currency

            UtxoCoin.Account.GetShowableBalance
                utxoAccount
                mode
                cancelSourceOption
        | _ ->
    ...
    let GetShowableBalance
        ...
        async {
            if Config.NoNetworkBalanceForDebuggingPurposes then
                ...
                let! maybeBalance =
                    GetShowableBalanceInternal account mode cancelSourceOption

                match maybeBalance with
                | None ->
                    return
                        NotFresh(
                            Caching.Instance.RetrieveLastCompoundBalance
    ...
    let internal GetAccountFromFile
        accountFile
        (currency: Currency)
        kind
        : IAccount =
        if currency.IsUtxo() then
            ...
            failwith
            <| SPrintF1 "Currency (%A) not supported for this API" currency
    ...
    let GetArchivedAccountsWithPositiveBalance
        (cancelSourceOption: Option<CustomCancelSource>)
            ...
            seq<Async<ArchivedAccount * Option<decimal>>> {
                ...
                for currency in allCurrencies do
                    let fromUnencryptedPrivateKeyToPublicAddressFunc =
                        if currency.IsUtxo() then
                            UtxoCoin.Account.GetPublicAddressFromUnencryptedPrivateKey
                                currency
                        elif currency.IsEtherBased() then
                    ...
                    let fromConfigAccountFileToPublicAddressFunc
                        (accountConfigFile: FileRepresentation)
                        =
                        let privateKeyFromConfigFile =
                            accountConfigFile.Content()

                        fromUnencryptedPrivateKeyToPublicAddressFunc
                            privateKeyFromConfigFile

                    for accountFile in
                        Config.GetAccountFiles [ currency ] AccountKind.Archived do
                        let account =
                            ArchivedAccount(
                                currency,
                                accountFile,
                                fromConfigAccountFileToPublicAddressFunc
                            )

                        let maybeBalanceJob =
                            GetShowableBalanceInternal
                                account
                        ...
                        yield
                            async {
                                let! maybeBalance =
                                    maybeBalanceJob cancelSourceOption
        ...
        async {
            ...
            return
                seq {
                    for account, maybePositiveBalance in
                        accountAndPositiveBalances do
    ...
    let EstimateFee
        ...
        async {
            ...
            | :? UtxoCoin.IUtxoAccount as utxoAccount ->
                ...
                let! fee =
                    UtxoCoin.Account.EstimateFee utxoAccount amount destination
    ...
    let private SaveOutgoingTransactionInCache
        transactionProposal
        (fee: IBlockchainFeeInfo)
        txId
        =
    ...
    let private CheckIfOutOfGas
        ...
        async {
            ...
            | :? Ether.TransactionMetadata as etherTxMetadata ->
                try
                    let! outOfGas =
                        Ether.Server.IsOutOfGas
                            transactionMetadata.Currency
                            txHash
                            etherTxMetadata.Fee.GasLimit

                    if outOfGas then
                        return
                            failwith
                            <| SPrintF1 "Transaction ran out of gas: %s" txHash
                with
                | ex ->
                    return
                        raise
    ...
    let BroadcastTransaction(trans: SignedTransaction<_>) : Async<Uri> =
        async {
            ...
            SaveOutgoingTransactionInCache
                trans.TransactionInfo.Proposal
    ...
    let SignTransaction
        ...
        | :? UtxoCoin.TransactionMetadata as btcTxMetadata ->
            ...
            | _ ->
                failwith
                    "An UtxoCoin.TransactionMetadata should come with a UtxoCoin.Account"

        | _ -> failwith "fee type unknown"

    let CheckValidPassword
        (password: string)
        (accountOpt: Option<NormalAccount>)
        =
        ...
        | _ ->
            let checkJobs =
                seq {
                    for account in
                        GetAllActiveAccounts().OfType<NormalAccount>() do
                        yield
                            async {
                                return
                                    checkValidPasswordForSingleAccount account
                            }
                }
    ...
    let private CreateArchivedAccount
        (currency: Currency)
        ...
        let fromConfigFileToPublicAddressFunc
            (accountConfigFile: FileRepresentation)
            =
        ...
        let fileName =
            fromUnencryptedPrivateKeyToPublicAddressFunc unencryptedPrivateKey
        ...
        let conceptAccount =
            {
                ...
                ExtractPublicAddressFromConfigFileFunc =
                    fromConfigFileToPublicAddressFunc
            }

        let newAccountFile =
            Config.AddAccount conceptAccount AccountKind.Archived
    ...
    let Archive (account: NormalAccount) (password: string) : unit =
        ...
        let privateKeyAsString =
            if currency.IsUtxo() then
                ...
                privKey
                    .GetWif(UtxoCoin.Account.GetNetwork currency)
                    .ToWif()
            elif currency.IsEther() then
    ...
    let SweepArchivedFunds
        ...
        | :? UtxoCoin.TransactionMetadata as utxoTxMetadata ->
            ...
            | :? UtxoCoin.ArchivedUtxoAccount as utxoAccount ->
                UtxoCoin.Account.SweepArchivedFunds
                    utxoAccount
                    balance
                    destination
                    utxoTxMetadata
            | _ ->
                failwith
                    "If tx metadata is UTXO type, archived account should be too"
        ...
        async {
            ...
            let! txId =
                ...
                | :? UtxoCoin.TransactionMetadata as btcTxMetadata ->
                    ...
                    | _ ->
                        failwith
                            "Account not Utxo-type but tx metadata is? report this bug (sendpayment)"

                | :? Ether.TransactionMetadata as etherTxMetadata ->
                    if not(currency.IsEtherBased()) then
                        failwith
                            "Account not ether-type but tx metadata is? report this bug (sendpayment)"

                    Ether.Account.SendPayment
                        account
                        etherTxMetadata
                        destination
                        amount
        ...
        let json =
            ...
            | t when t = typeof<Ether.TransactionMetadata> ->
                let unsignedEthTx =
                    {
                        Metadata =
                            box trans.TransactionInfo.Metadata
    ...
    let CreateReadOnlyAccounts(watchWalletInfo: WatchWalletInfo) : Async<unit> =
        async {
            for etherCurrency in
                Currency
                    .GetAll()
                    .Where(fun currency -> currency.IsEtherBased()) do
                do!
                    ValidateAddress
                        etherCurrency
                        watchWalletInfo.EtherPublicAddress
                ...
                Config.AddAccount
                    conceptAccountForReadOnlyAccount
                    AccountKind.ReadOnly
            ...
            for utxoCurrency in
                Currency
                    .GetAll()
                    .Where(fun currency -> currency.IsUtxo()) do
                let address =
                        ...
                        FileRepresentation =
                            {
                                Name = address
                                Content =
                                    fun _ -> watchWalletInfo.UtxoCoinPublicKey
                            }
                        ExtractPublicAddressFromConfigFileFunc =
                            (fun file -> file.Name)
                    }
    ...
    let private CreateConceptEtherAccountInternal
        ...
        async {
            ...
            return
                virtualFile, Ether.Account.GetPublicAddressFromNormalAccountFile
        }
    ...
    let private CreateConceptAccountInternal
        ...
        async {
            if currency.IsUtxo() then
                let! virtualFile =
                    UtxoCoin.Account.Create currency password seed

                return
                    virtualFile,
                    UtxoCoin.Account.GetPublicAddressFromNormalAccountFile
                        currency
            elif currency.IsEtherBased() then
            ...
            return
                {
                    ...
                    ExtractPublicAddressFromConfigFileFunc =
                        fromEncPrivKeyToPublicAddressFunc
                }
        }
    ...
    let GenerateMasterPrivateKey
        ...
        async {
            let salt =
                SPrintF2
                    "%s+%s"
                    (dobPartOfSalt.Date.ToString("yyyyMMdd"))
                    (emailPartOfSalt.ToLower())

            let privateKeyBytes = WarpKey.CreatePrivateKey passphrase salt
            return privateKeyBytes
        }

    let CreateAllConceptAccounts
        (privateKeyBytes: array<byte>)
        (encryptionPassword: string)
        : Async<seq<ConceptAccount>> =
        async {
            let etherAccounts =
                CreateEtherNormalAccounts encryptionPassword privateKeyBytes

            let nonEthCurrencies =
                Currency
                    .GetAll()
                    .Where(fun currency -> not(currency.IsEtherBased()))
    ...
    let CreateAllAccounts
        (privateKeyBytes: array<byte>)
        (encryptionPassword: string)
        : Async<unit> =
        async {
            let! allConceptAccounts =
                CreateAllConceptAccounts privateKeyBytes encryptionPassword

            for conceptAccount in allConceptAccounts do
                CreateNormalAccount conceptAccount |> ignore<NormalAccount>
        }
    ...
    let CheckValidSeed
        ...
        async {
            ...
            let! allConceptAccounts =
                CreateAllConceptAccounts
                    masterPrivateKey
                    (Guid.NewGuid().ToString())

            return
                allConceptAccounts.All(fun conceptAccount ->
                    GetAllActiveAccounts()
                        .Any(fun account ->
                            ...
                            let publicAddressMatches =
                                (account.PublicAddress = publicAddressOfConceptAccount)
    ...
    let private SerializeUnsignedTransactionPlain
        ...
        match txMetadata with
        | :? Ether.TransactionMetadata as etherTxMetadata ->
            Ether.Account.SaveUnsignedTransaction
                transProposal
                etherTxMetadata
                readOnlyAccounts
        | :? UtxoCoin.TransactionMetadata as btcTxMetadata ->
            UtxoCoin.Account.SaveUnsignedTransaction
                transProposal
                btcTxMetadata
                readOnlyAccounts
        | _ -> failwith "fee type unknown"
    ...
    let public ImportUnsignedTransactionFromJson
        (json: string)
        : UnsignedTransaction<IBlockchainFeeInfo> =

        let transType = Marshalling.ExtractType json

        match transType with
        | _ when
            transType = typeof<UnsignedTransaction<UtxoCoin.TransactionMetadata>>
            ->
        ...
        | _ when
            transType = typeof<UnsignedTransaction<Ether.TransactionMetadata>>
            ->
            ...
            deserializedBtcTransaction.ToAbstract()
        | _ when
            transType.GetGenericTypeDefinition() = typedefof<SignedTransaction<_>>
            ->
        ...
        | unexpectedType ->
            ...
            <| Exception(
                SPrintF1
                    "Unknown unsignedTransaction subtype: %s"
                    unexpectedType.FullName
            )

    let public ImportSignedTransactionFromJson
        (json: string)
        : SignedTransaction<IBlockchainFeeInfo> =
        let transType = Marshalling.ExtractType json

        match transType with
        | _ when
            transType = typeof<SignedTransaction<UtxoCoin.TransactionMetadata>>
            ->
            ...
            deserializedBtcTransaction.ToAbstract()
        | _ when
            transType = typeof<SignedTransaction<Ether.TransactionMetadata>>
            ->
            ...
            deserializedBtcTransaction.ToAbstract()
        | _ when
            transType.GetGenericTypeDefinition() = typedefof<UnsignedTransaction<_>>
            ->
        ...
        | unexpectedType ->
            ...
            <| Exception(
                SPrintF1
                    "Unknown signedTransaction subtype: %s"
                    unexpectedType.FullName
            )
    ...
    let LoadUnsignedTransactionFromFile
        (filePath: string)
        : UnsignedTransaction<IBlockchainFeeInfo> =
    ...
    let GetSignedTransactionDetails<'T when 'T :> IBlockchainFeeInfo>
        (signedTransaction: SignedTransaction<'T>)
        : ITransactionDetails =
        let currency =
            signedTransaction.TransactionInfo.Proposal.Amount.Currency
        ...
        if currency.IsUtxo() then
            ...
            Ether.Account.GetSignedTransactionDetails signedTransaction
        else
            failwith <| (SPrintF1 "Unknown currency: %A" currency)
