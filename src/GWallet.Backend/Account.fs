...
module Account =
        ...
        let asyncJobs =
            seq<Async<ArchivedAccount * Option<decimal>>> {
                ...
                for currency in allCurrencies do
                    ...
                    for accountFile in Config.GetAccountFiles
                                           [ currency ]
                                           AccountKind.Archived do
                        let account =
                            ArchivedAccount (
                                currency,
                                accountFile,
    ...
    let CreateAllAccounts
        (masterPrivateKeyTask: Task<array<byte>>)
        (encryptionPassword: string)
        : Async<unit> =
        async {
