            ...
            seq {
                for (KeyValue(currency, currencyBalances)) in self.Balances do
                    for (KeyValue(address, (balance, _))) in currencyBalances do
                        if
                            readOnlyAccounts.Any(fun account ->
                                (account :> IAccount).PublicAddress = address
                            ) then
...
module Caching =
    ...
    let private LoadFromDisk
        ...
        let maybeFirstRun, resultingNetworkData =
            ...
            | Some networkData ->
                if
                    WeirdNullCheckToDetectVersionConflicts
    ...
    let MapCombinations<'K, 'V when 'K: comparison>
        (map: Map<'K, 'V>)
        : List<List<'K * 'V>> =
    ...
    type MainCache
        (
            maybeCacheFiles: Option<CacheFiles>,
        ...
        let SaveNetworkDataToDisk(newCachedData: CachedNetworkData) =
            ...
            File.WriteAllText(
                cacheFiles.CachedNetworkData.FullName,
                networkDataInJson
            )
        ...
        let SaveServerRankingsToDisk
            (serverStats: ServerRanking)
            : ServerRanking =
            let serverStatsInJson = ServerRegistry.Serialize serverStats

            // it is assumed that SaveToDisk is being run under a lock() block
            File.WriteAllText(
                cacheFiles.ServerStats.FullName,
                serverStatsInJson
            )

            match LoadFromDiskInternal<ServerRanking> cacheFiles.ServerStats
                with
        ...
        member __.ClearAll() =
            ...
            SaveServerRankingsToDisk Map.empty |> ignore<ServerRanking>
        ...
        member __.StoreLastFiatUsdPrice
            (
                currency,
                lastFiatUsdPrice: decimal
            ) : unit =
            lock
                cacheFiles.CachedNetworkData
                (fun _ ->
            ...
            lock
                ...
                (fun _ ->
                    let balance =
                        try
                            Cached(
                                (sessionCachedNetworkData.Balances.Item currency)
                                    .Item address
                            )
                        with
                        // FIXME: rather use tryFind func instead of using a try-with block
        ...
        member self.TryRetrieveLastCompoundBalance
            (address: PublicAddress)
            (currency: Currency)
            : Option<decimal> =
            let maybeCachedBalance =
        ...
        member __.RetrieveAndUpdateLastCompoundBalance
            (address: PublicAddress)
            (currency: Currency)
            (newBalance: decimal)
            : CachedValue<decimal> =
                ...
                (fun _ ->
                    let newCachedValueWithNewBalance, previousBalance =
                        let newCurrencyBalances, previousBalance =
                            match
                                sessionCachedNetworkData.Balances.TryFind
                                    currency
                                with
                            | None -> Map.empty, None
                    ...
                    let newCachedValueWithNewBalanceAndMaybeLessTransactions =
                        let maybeNewValue =
                            FSharpUtil.option {
                                ...
                                do!
                                    if newBalance <> previousCachedBalance
        ...
        member private __.StoreTransactionRecord
            (address: PublicAddress)
            (currency: Currency)
            (txId: string)
            (amount: decimal)
            ...
            lock
                ...
                (fun _ ->
                    let newCurrencyAddresses =
                        match
                            sessionCachedNetworkData.OutgoingTransactions.TryFind
                                currency
                            with
                        | None -> Map.empty
        ...
        member self.StoreOutgoingTransaction
            (address: PublicAddress)
        ...
        member __.SaveServerLastStat
            (serverMatchFunc: ServerDetails -> bool)
            (stat: HistoryFact)
            : unit =
            lock
        ...
        member __.BootstrapServerStatsFromTrustedSource() : Async<unit> =
            let downloadFile url : Async<Option<string>> =
                ...
                async {
                    ...
                    | ex ->
                        Infrastructure.ReportWarning ex |> ignore<bool>
                        return None
                }
