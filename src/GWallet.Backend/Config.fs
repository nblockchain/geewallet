...
module Config =
    ...
    // still used by Mobile & Windows for now
    let private GetLegacyConfigDirForThisProgram() =
    ...
    // used by Unix (macOS & Linux) now
    let private GetModernConfigDirForThisProgram() =
    ...
    // TODO: use Fsdk's CopyDirectoryRecursively() func?
    ...
    // its contents migrated, if any; NOTE: it's public because it is unit tested
    let public MaybeMigrateOldConfigDir (newConfigDir: DirectoryInfo)
    ...
    // Workaround for https://github.com/nblockchain/geewallet/issues/312 : .NETv8 (or
    // newer) changed the folder that Environment.SpecialFolder.ApplicationData points at on
    // macOS (it used to be $HOME/.config, and it's now a subfolder of $HOME/Library), which
    // meant that Backend stopped finding its config folder when its .NET runtime got upgraded
    // from .NETv6 to .NETv8. Windows and mobile platforms (Android & iOS) keep using the
    ...
    let internal GetConfigDirForThisProgram(): DirectoryInfo =
        if IsWindowsPlatform() || IsMobilePlatform() then
            GetLegacyConfigDirForThisProgram()
        else
            ...
            lock configDirResolutionLock (fun _ ->
                let configDir = GetModernConfigDirForThisProgram()
                ...
                if not (HasExistingConfig configDir) then
                    let oldConfigDirCandidates = seq {
                        ...
                        yield GetLegacyConfigDirForThisProgram()
