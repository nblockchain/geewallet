...
module Config =
    ...
    let GetMonoVersion (): Option<Version> =
        FSharpUtil.option {
            ...
            let simpleVersion =
                fullVersion.Substring (0, fullVersion.IndexOf (' ')) |> Version
    ...
    let private GetConfigDir (currency: Currency) (accountKind: AccountKind) =
        ...
        let configDir =
            Path.Combine (baseConfigDir, currency.ToString ()) |> DirectoryInfo
        ...
        for accountKind in (AccountKind.All ()) do
            ...
            for token in Currency.GetAll () do
                if token.IsEthToken () then
                    ...
                    for ethAccountFilePath in Directory.GetFiles
                                                  ethConfigDir.FullName do
                        let newPath =
                            ethAccountFilePath.Replace (
                                ethConfigDir.FullName,
                                tokenConfigDir.FullName
                            )
    ...
    let GetAccountFiles
        ...
        : seq<FileRepresentation> =
        seq {
            for currency in currencies do
                for filePath in Directory.GetFiles
                                    (GetConfigDir currency accountKind).FullName do
    ...
    let private GetFile (currency: Currency) (account: BaseAccount): FileInfo =
        let configDir, fileName =
            GetConfigDir currency account.Kind, account.AccountFile.Name
    ...
    let AddAccount
        ...
        let newAccountFile =
            Path.Combine (
                configDir.FullName,
                conceptAccount.FileRepresentation.Name
            )
            |> FileInfo
        ...
        File.WriteAllText (
            newAccountFile.FullName,
            conceptAccount.FileRepresentation.Content ()
        )
    ...
    let private RemoveAccount (account: BaseAccount): unit =
        ...
        if not configFile.Exists then
            failwith
            <| SPrintF1
                "File %s doesn't exist. Please report this issue."
                configFile.FullName
        else
            configFile.Delete ()
