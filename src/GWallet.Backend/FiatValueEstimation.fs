    ...
    type BitPayProvider = JsonProvider<"""
{
    ...
    let private RetrieveOnline currency = async {
        ...
        let allJobs = FSharpUtil.AsyncExtensions.MixedParallel3 bitPayJob coinGeckoJob coinCapJob
        let! maybeUsdPriceFromBitPay, maybeUsdPriceFromCoinGecko, maybeUsdPriceFromCoinCap = allJobs
        let someUsdPrice, otherUsdPrice =
            match maybeUsdPriceFromBitPay, maybeUsdPriceFromCoinGecko, maybeUsdPriceFromCoinCap with
            | None, None, None ->
                None, None
            | None, Some foo, None
            | Some foo, None, None
            ...
            | Some foo, Some bar, Some baz ->
                let sorted = List.sort [foo; bar; baz]
        ...
        let result =
            ...
            | None, Some usdPrice ->
                Some usdPrice
            | Some foo, Some bar ->
                let higher = Math.Max(foo, bar)
                let lower = Math.Min(foo, bar)

                // example: 100USD vs: 66.666USD (or lower)
                let abnormalDifferenceRate = 1.5m
                if (higher / lower) > abnormalDifferenceRate then
                    let err =
                        SPrintF4 "Alert: difference of USD exchange rate (for %A) between the closest providers is abnormally high: %M vs %M (H/L > %M)"
                            currency
                            higher
                            lower
