    ...
    let private GetEtherServers (currency: Currency) : List<ServerDetails> =
        let baseCurrency =
            if currency = Currency.ETC || currency = Currency.ETH then
                ...
                failwith
                <| SPrintF1
                    "Assertion failed: Ether currency %A not supported?"
                    currency
...
module Server =
    ...
    let private Web3Server (connectionTimeOut, serverDetails: ServerDetails) =
        ...
        | {
            ...
            failwith
            <| SPrintF1
                "Ether server of TCP connection type?: %s"
                serverDetails.ServerInfo.NetworkPath
        | {
              Protocol = Http
              Encrypted = encrypted
          } ->
            ...
            let uri =
                SPrintF2 "%s://%s" protocol serverDetails.ServerInfo.NetworkPath

            SomeWeb3 (connectionTimeOut, uri)

    let HttpRequestExceptionMatchesErrorCode
        (ex: Http.HttpRequestException)
        (errorCode: int)
        : bool =
    ...
    let PerformEtherRemoteCallWithTimeout<'T, 'R> (job: Async<'R>) : Async<'R> =
        async {
            let! maybeResult =
                FSharpUtil.WithTimeout Config.DEFAULT_NETWORK_TIMEOUT job
            ...
            | None ->
                return
                    ...
                    <| ServerTimedOutException
                        "Timeout when trying to communicate with Ether server"
    ...
    let MaybeRethrowWebException (ex: Exception) : unit =
        ...
        | Some webEx ->
            ...
            if webEx.Status = WebExceptionStatus.SecureChannelFailure then
                raise
                <| ServerChannelNegotiationException (
                    exMsg,
                    webEx.Status,
                    webEx
                )
    ...
    let MaybeRethrowHttpRequestException (ex: Exception) : unit =
        let maybeHttpReqEx =
            FSharpUtil.FindException<Http.HttpRequestException> ex

        match maybeHttpReqEx with
        | Some httpReqEx ->
            if HttpRequestExceptionMatchesErrorCode
                httpReqEx
                (int CloudFlareError.ConnectionTimeOut) then
                raise <| ServerTimedOutException (exMsg, httpReqEx)

            if HttpRequestExceptionMatchesErrorCode
                httpReqEx
                (int CloudFlareError.OriginUnreachable) then
                raise <| ServerTimedOutException (exMsg, httpReqEx)

            if HttpRequestExceptionMatchesErrorCode
                httpReqEx
                (int CloudFlareError.OriginSslHandshakeError) then
                raise
                <| ServerChannelNegotiationException (
                    exMsg,
                    CloudFlareError.OriginSslHandshakeError,
                    httpReqEx
                )

            if HttpRequestExceptionMatchesErrorCode
                httpReqEx
                (int CloudFlareError.WebServerDown) then
                raise
                <| ServerUnreachableException (
                    exMsg,
                    CloudFlareError.WebServerDown,
                    httpReqEx
                )

            if HttpRequestExceptionMatchesErrorCode
                httpReqEx
                (int CloudFlareError.OriginError) then
                raise
                <| ServerUnreachableException (
                    exMsg,
                    CloudFlareError.OriginError,
                    httpReqEx
                )

            if HttpRequestExceptionMatchesErrorCode
                httpReqEx
                (int HttpStatusCode.BadGateway) then
                raise
                <| ServerUnreachableException (
                    exMsg,
            ...
            if HttpRequestExceptionMatchesErrorCode
                httpReqEx
                (int HttpStatusCode.GatewayTimeout) then
                raise
                <| ServerUnreachableException (
                    exMsg,
            ...
            if HttpRequestExceptionMatchesErrorCode
                httpReqEx
                (int HttpStatusCode.ServiceUnavailable) then
            ...
            if HttpRequestExceptionMatchesErrorCode
                httpReqEx
                (int HttpStatusCode.Forbidden) then
                raise <| ServerMisconfiguredException (exMsg, httpReqEx)

            if HttpRequestExceptionMatchesErrorCode
                httpReqEx
                (int HttpStatusCode.Unauthorized) then
                raise <| ServerMisconfiguredException (exMsg, httpReqEx)

            if HttpRequestExceptionMatchesErrorCode
                httpReqEx
                (int HttpStatusCode.MethodNotAllowed) then
                raise <| ServerMisconfiguredException (exMsg, httpReqEx)

            if HttpRequestExceptionMatchesErrorCode
                httpReqEx
                (int HttpStatusCode.InternalServerError) then
                raise <| ServerUnavailableException (exMsg, httpReqEx)

            if HttpRequestExceptionMatchesErrorCode
                httpReqEx
                (int HttpStatusCode.NotFound) then
                raise <| ServerUnavailableException (exMsg, httpReqEx)
            ...
            if HttpRequestExceptionMatchesErrorCode
                httpReqEx
                (int HttpStatusCode.BadRequest) then
                raise
            ...
            if HttpRequestExceptionMatchesErrorCode
                httpReqEx
                (int HttpStatusCodeNotPresentInTheBcl.TooManyRequests) then
                raise <| ServerRestrictiveException (exMsg, httpReqEx)

            if HttpRequestExceptionMatchesErrorCode
                httpReqEx
                (int HttpStatusCodeNotPresentInTheBcl.FrozenSite) then
                raise <| ServerUnavailableException (exMsg, httpReqEx)
    ...
    let MaybeRethrowRpcResponseException (ex: Exception) : unit =
        let maybeRpcResponseEx =
            FSharpUtil.FindException<JsonRpcSharp.Client.RpcResponseException>
                ex
        ...
        | Some rpcResponseEx ->
            if rpcResponseEx.RpcError <> null then
                ...
                | a when
                    a = int
                            RpcErrorCode.StatePruningNodeOrMissingTrieNodeOrHeaderNotFound
                    ->
                    if
                        not
                            (
                                err32kPossibleMessages.Any (fun msg ->
                                    rpcResponseEx.RpcError.Message.Contains msg
                                )
                            )
                    then
                        let possibleErrMessages =
                            SPrintF1
                                "'%s'"
                                (String.Join ("' or '", err32kPossibleMessages))

                        raise
                        <| Exception (
                            SPrintF3
                                "Expecting %s in message of a %d code, but got '%s'"
                                possibleErrMessages
                                (int
                                    RpcErrorCode.StatePruningNodeOrMissingTrieNodeOrHeaderNotFound)
                        ...
                        raise
                        <| ServerMisconfiguredException (exMsg, rpcResponseEx)
    ...
    let MaybeRethrowRpcClientTimeoutException (ex: Exception) : unit =
        let maybeRpcTimeoutException =
            FSharpUtil.FindException<JsonRpcSharp.Client.RpcClientTimeoutException>
                ex
    ...
    let MaybeRethrowNetworkingException (ex: Exception) : unit =
        let maybeSocketRewrappedException =
            Networking.FindExceptionToRethrow ex exMsg
    ...
    let MaybeRethrowObjectDisposedException (ex: Exception) : unit =
        let maybeRpcUnknownEx =
            FSharpUtil.FindException<JsonRpcSharp.Client.RpcClientUnknownException>
                ex

        match maybeRpcUnknownEx with
        | Some _ ->
            let maybeObjectDisposedEx =
                FSharpUtil.FindException<ObjectDisposedException> ex
    ...
    let MaybeRethrowInnerRpcException (ex: Exception) : unit =
        ...
        | Some rpcUnknownEx ->
            ...
            let maybeDeSerializationEx =
                FSharpUtil.FindException<JsonRpcSharp.Client.DeserializationException>
                    rpcUnknownEx
            ...
            | Some httpReqEx ->
                if httpReqEx.Message.Contains "SSL" then
                    ...
                    | None ->
                        let maybeSecEx =
                            FSharpUtil.FindException<System.Security.Authentication.AuthenticationException>
                                ex
    ...
    let private FaultTolerantParallelClientInnerSettings
        ...
        let retries =
            ...
            | Currency.ETC when etcEcosystemIsMomentarilyCentralized ->
                Config.NUMBER_OF_RETRIES_TO_SAME_SERVERS * 2u
...

        {
            ...
            ExceptionHandler =
                Some (fun ex -> Infrastructure.ReportWarning ex |> ignore<bool>)
            ResultSelectionMode =
                Selective
                    {
    ...
    let private FaultTolerantParallelClientDefaultSettings
        ...
        FaultTolerantParallelClientInnerSettings
            numberOfConsistentResponsesRequired
            mode
            currency
    ...
    let GetServerFuncs<'R>
        ...
        let serverFuncs =
            Seq.map (Web3ServerToGenericServer web3Func) etherServers
    ...
    let private GetRandomizedFuncs<'R>
        ...
        GetServerFuncs web3Func etherServers currency |> List.ofSeq
    ...
    let private NUMBER_OF_CONFIRMATIONS_TO_CONSIDER_BALANCE_CONFIRMED =
        BigInteger (45)

    let private GetBlockToCheckForConfirmedBalance
        (web3: Web3)
    ...
    let private GetConfirmedEtherBalanceInternal
        (web3: Web3)
        (publicAddress: string)
        : Async<HexBigInteger> =
        async {
            let! blockForConfirmationReference =
                GetBlockToCheckForConfirmedBalance web3
            (*
...
*)
    ...
    let private BalanceMatchWithCacheOrInitialBalance
        address
        currency
        someRetrievedBalance
        =
        if Caching.Instance.FirstRun then
            someRetrievedBalance = 0m
        else
            match Caching.Instance.TryRetrieveLastCompoundBalance
                      address
                      currency with
            | None -> false
    ...
    let private GetConfirmedTokenBalanceInternal
        (web3: Web3)
        (publicAddress: string)
        (currency: Currency)
        : Async<decimal> =
        ...
        async {
            let web3Funcs =
                let web3Func (web3: Web3) : Async<decimal> =
                    ...
                    | BalanceType.Unconfirmed ->
                        let tokenService =
                            TokenManager.TokenServiceWrapper (web3, currency)
        ...
        async {
            let web3Funcs =
                let web3Func (web3: Web3) : Async<HexBigInteger> =
                    ...
                    let contractHandler =
                        web3.Eth.GetContractHandler contractAddress
                    ...
                    async {
                        ...
                        let task =
                            contractHandler.EstimateGasAsync<TransferFunction> (
                                transferFunctionMsg,
                                cancelToken
                            )
    ...
    let private AverageGasPrice
        (gasPricesFromDifferentServers: List<HexBigInteger>)
        : HexBigInteger =
        let sum =
        ...
        let avg =
            BigInteger.Divide (
                sum,
                BigInteger (gasPricesFromDifferentServers.Length)
            )
                ...
                let web3Func (web3: Web3) : Async<HexBigInteger> =
                    async {
                        ...
                        if hexBigInteger.Value = BigInteger 0 then
                            return
                                failwith
                                    "Some server returned zero for gas price, which is invalid"
    ...
    let BroadcastTransaction
        ...
        async {
            ...
            with
            | ex ->
                match FSharpUtil.FindException<JsonRpcSharp.Client.RpcResponseException>
                          ex with
                ...
                | Some rpcResponseException ->
                    ...
                    if
                        rpcResponseException.Message.StartsWith
                            (
            ...
            let web3Funcs =
                let web3Func (web3: Web3) : Async<TransactionStatusDetails> =
                    async {
                        ...
                        let task =
                            web3.TransactionManager.TransactionReceiptService.PollForReceiptAsync (
                                txHash,
                                cancelToken
                            )
    ...
    let IsOutOfGas
        ...
        : Async<bool> =
        async {
            let! transactionStatusDetails =
                GetTransactionDetailsFromTransactionReceipt currency txHash
    ...
    let CheckIfAddressIsAValidPaymentDestination
        (currency: Currency)
        (address: string)
        : Async<unit> =
        async {
