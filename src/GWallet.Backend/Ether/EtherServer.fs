...
module Server =
    ...
    let MaybeRethrowRpcResponseException (ex: Exception): unit =
        ...
        | Some rpcResponseEx ->
            if rpcResponseEx.RpcError <> null then
                if rpcResponseEx.RpcError.Code = int
                                                     RpcErrorCode.StatePruningNodeOrMissingTrieNodeOrHeaderNotFound then
                    if (not (
                ...
                if (rpcResponseEx.RpcError.Code = int
                                                      RpcErrorCode.UnknownBlockNumber) then
    ...
    let private faultTolerantEtherClient =
        JsonRpcSharp.Client.HttpClient.ConnectionTimeout <-
