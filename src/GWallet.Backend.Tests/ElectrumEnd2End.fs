...
[<TestFixture>]
type ElectrumEnd2End() =

    member private __.CreateTestListener() =
    ...
    member private __.CreateStratumClient(port: uint32) =
        let jsonRpcClient = JsonRpcTcpClient("localhost", port)
        StratumClient(jsonRpcClient)

    [<Test>]
    // test for bug https://github.com/nblockchain/geewallet/issues/316
    member __.``hitting the NRE reported by the user``() =
        ...
        let listener, port = __.CreateTestListener()
        ...
        let ex =
            try
                let stratumClient = __.CreateStratumClient(port)
                async {
